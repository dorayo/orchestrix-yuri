      ...
      if (fs.existsSync(focusPath)) {
        try {
          ...
          if (phaseNum === 3) {
            ...
            const hasPhase3 = fs.existsSync(phase3Path);
            ...
            if (hasPhase3) {
              ...
              if (phase3.status === 'in_progress') {
                ...
                // Try to find tmux session from phase3 or by scanning tmux
                if (phase3.tmux && phase3.tmux.session) {
                  this.orchestrator._session = phase3.tmux.session;
                } else {
                  // Auto-detect: look for orchestrix-* tmux session
                  try {
                    ...
                    if (devSession) this.orchestrator._session = devSession;
                  } catch { /* no tmux */ }
                }
                ...
                try {
                  ...
                  log.warn(`Progress card failed: ${cardErr.message}`);
                }
            ...
            // If phase3.yaml missing or card failed, still try basic progress
            if (!cardGenerated) {
              // Scan stories directly even without phase3.yaml
              try {
                const { execSync } = require('child_process');
                ...
                if (fs.existsSync(scriptPath)) {
                  ...
                  if (output && output !== 'NO_STORIES_DIR') {
                    ...
                    for (const line of output.split('\n')) {
                      ...
                      total += n;
                      if (status === 'Done') done += n;
                    }
                    if (total > 0) {
                      const pct = Math.round(done / total * 100);
                      parts.push(`📊 Story: ${done}/${total} (${pct}%)`);
                    }
                  }
                }
              } catch { /* ok */ }
