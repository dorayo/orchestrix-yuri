...
class TelegramAdapter {
  ...
  async start() {
    ...
    // grammy's bot.start() returns a promise that only resolves on bot.stop().
    // We resolve on onStart callback (bot connected) so the gateway can
    // continue initializing other adapters (e.g., Feishu).
