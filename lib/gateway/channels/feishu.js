...
class FeishuAdapter {
  ...
  async start() {
    ...
    // Suppress SDK's verbose info logs during initialization.
    // The SDK ignores loggerLevel/logger options and uses console directly.
    const _origLog = console.log;
    const _origInfo = console.info;
    console.log = (...args) => {
      const s = args.map(String).join(' ');
      if (s.includes('[info]') || s.includes('client ready') || s.includes('event-dispatch') || s.includes('[ws]')) return;
    ...
    // Restore console after SDK init
