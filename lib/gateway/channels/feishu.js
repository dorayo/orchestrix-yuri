...
class FeishuAdapter {
  ...
  async start() {
    ...
    // Suppress SDK's own info/warn logs with a silent logger
    const silentLogger = {
      error: (...args) => log.error(`Feishu SDK: ${args.join(' ')}`),
      warn: () => {},
      info: () => {},
      debug: () => {},
      trace: () => {},
    ...
    const sdkLogOpts = { logger: silentLogger, loggerLevel: lark.LoggerLevel.ERROR };
