...
class FeishuAdapter {
  ...
  async start() {
    ...
    // Permanently suppress SDK's verbose info logs.
    // The SDK ignores loggerLevel/logger options and uses console directly.
    // Async events (like 'ws client ready') fire after start() returns,
    // so we keep the filter active for the lifetime of the process.
    if (!FeishuAdapter._consolePatched) {
      const _origLog = console.log;
      const _origInfo = console.info;
      console.log = (...args) => {
        const s = JSON.stringify(args);
        if (s.includes('[info]') || s.includes('[ws]') || s.includes('client ready') || s.includes('event-dispatch')) return;
      ...
      FeishuAdapter._consolePatched = true;
    }
