...
const { loadL1Context, resolveProjectRoot, findClaudeBinary } = require('./claude-cli');
 ...
 * Same interface as claude-cli.callClaude for drop-in replacement.
 *
 * @param {object} opts
...
/**
 * Compose prompt for persistent session.
 * Unlike claude-cli, we only send the raw user message —
 * the session already has L1 context from initialization.
 *
 * @param {string} userMessage - The user's message text
 * @param {Array} chatHistory - Ignored in persistent mode (Claude keeps its own context)
 * @returns {string}
 */
